...
def _require_wallet(value: str | None, *, subject: str = "Session") -> Pubkey:
    """Resolve a base58 wallet or fail with a useful 4xx.

    `CurrentUser` exposes `wallet` (see app/auth.py) — not `wallet_address`, which
    earlier code assumed and which raised AttributeError at request time.
    """
    if not value:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=f"{subject} has no wallet address on record",
        )
    try:
        ...
        raise HTTPException(
            ...
            detail=f"{subject} wallet address is malformed",
        ) from exc
...
async def get_create_plan_transaction(
    ...
    creator_pubkey = _require_wallet(user.wallet)
...
async def get_purchase_subscription_transaction(
    ...
    # AthleteProfile has no wallet of its own — the wallet lives on the linked
    # profiles row, so resolve it through the join rather than off the athlete.
    athlete_wallet = await session.scalar(
        select(Profile.wallet)
        .join(AthleteProfile, AthleteProfile.profile_id == Profile.id)
        .where(AthleteProfile.id == plan.athlete_profile_id)
    )

    supporter_pubkey = _require_wallet(user.wallet)
    athlete_pubkey = _require_wallet(athlete_wallet, subject="Athlete")
