...
export function CampaignEditorPage() {
  ...
  return <div className="mx-auto max-w-3xl space-y-6"><div><Link className="text-sm text-muted-foreground hover:text-foreground" to="/athlete">← Athlete workspace</Link><h1 className="mt-3 text-2xl font-bold tracking-wide">{campaign ? 'Edit campaign draft' : 'New campaign'}</h1>{message && <p className="mt-2 text-sm text-destructive">{message}</p>}</div><CampaignForm planId={plan.id} initialCampaign={campaign} busy={busy} onSave={save} /></div>
}
