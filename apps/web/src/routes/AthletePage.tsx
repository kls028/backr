  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card><CardHeader><CardTitle>Campaign</CardTitle></CardHeader><CardContent className="space-y-3"><p className="text-sm text-muted-foreground">Create or resume a campaign draft.</p><Button asChild disabled={!plan || plan.status !== 'published'}><Link to="/athlete/campaigns/new">New campaign</Link></Button>{plan?.status !== 'published' && <p className="text-xs text-muted-foreground">Publish a plan before creating a campaign.</p>}</CardContent></Card>
      </div>
